...
type Categoria = { value: string; label: string; productos: string[] };

// Catálogo real de ventas (~12k filas): cambia poco, se cachea en memoria
// para no repaginar la tabla completa en cada carga del panel de mermas.
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, categorias: cache.data });
  } catch (e) {
